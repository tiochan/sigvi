...
*/
	...
	function alerts_show_servers_status($show_header= true, $show_content=true, $show_all_servers=false) {
		...
		$query_high_audit="select high from audit where id_server = %d";
		$query_medium_audit="select medium from audit where id_server = %d";
		$query_low_audit="select low from audit where id_server = %d";
		...
		$server_status_table.= "
			...
			<table class='data_box_external' width='600'><tr><td colspan='3'>
			<table class='data_box_rows' width='100%' cellspacing='0'>
				<tr><th class='data_box_rows'rowspan=2 style='text-align:center;'><b>" . $MESSAGES['SERVER'] . "</b></th>
				<th class='data_box_rows' colspan=4 style='text-align:center;'><b>" . $MESSAGES["VULN_SOURCES"] . "</b></th>
				<th class='data_box_rows' colspan=4 style='text-align:center;'><b>" . $MESSAGES['AUDIT'] . "</b></th></tr>
            	<tr>
...
";
		...
		while($row=$global_db->dbms_fetch_row($res)) {
		    ...
		    $query_tmp= sprintf($query_high_audit, $server_id);
			$res2= $global_db->dbms_query($query_tmp);
			if($global_db->dbms_check_result($res2)) {
				$row2= $global_db->dbms_fetch_row($res2);
				$high_audit= $row2[0];
			...
			$query_tmp= sprintf($query_medium_audit, $server_id);
			$res2= $global_db->dbms_query($query_tmp);
			if($global_db->dbms_check_result($res2)) {
				$row2= $global_db->dbms_fetch_row($res2);
				$med_audit= $row2[0];
			...
			$query_tmp= sprintf($query_low_audit, $server_id);
			$res2= $global_db->dbms_query($query_tmp);
			if($global_db->dbms_check_result($res2)) {
				$row2= $global_db->dbms_fetch_row($res2);
				$low_audit= $row2[0];
			...
			$total= $high + $med + $low;
			$total_audit= $high_audit + $med_audit + $low_audit;
			if($show_all_servers or ($total > 0)) {
				...
				<tr class='data_box_rows_tabular_odd'>
					...
					<td class='data_box_cell' bgcolor='" . FAS_HIGH_SEVERITY_COLOR . "'>" . $high_audit . "</TD>
					<td class='data_box_cell' bgcolor='" . FAS_MED_SEVERITY_COLOR . "'>" . $med_audit . "</TD>
					<td class='data_box_cell' bgcolor='" . FAS_LOW_SEVERITY_COLOR . "'>" . $low_audit . "</TD>
					<td class='data_box_cell'>" . $total_audit . "</TD>
				</tr>\n";
			}
		}

...
*/
	...
	class dw_users extends datawindow {
		...
		public function dw_users(&$optional_db=null) {
			...
			$fields[12]= new field("receive_all",$MESSAGES["USER_FIELD_RECEIVE_ALL"],"fbool",true,false,true,true,1);
			$fields[13]= new field("id_filter",$MESSAGES["FILTER_NOTIFICATION"],"foreign_key",false,false,true,true,null,$filter_reference);
		...
		function pre_update($row_id, $old_values, &$new_values) {
			...
			if($USER_LEVEL == 0) {
				...
				// Unset assigned alerts
				if($old_values["id_group"] != $new_values["id_group"]) $this->unset_alerts($row_id);
				return 1;
			}
		...
		private function unset_alerts($user_id) {
			global $MESSAGES;
			global $global_db;

			// Unset assigned alerts
			$query="update alerts set id_user=null where id_user = '$user_id'";
			$global_db->dbms_query($query);
			html_showInfo($MESSAGES["ALERT_ASSIGNED_TO_UNSET"]);
		}
	}
?>
 ...
 */
	...
	function send_vulnerability_report($vuln_array) {
		...
		if($global_db->dbms_check_result($res)) {
			while($row=$global_db->dbms_fetch_row($res)) {
				...
				$email= get_vulnerability_report($vuln_array, $id_user, true, $vulns_in_report);

				if(($vulns_in_report > 0) or SEND_VOID_VULN_REPORT) {
					send_mail($to, $subject, $subject, $email, DEFAULT_EMAIL_METHOD, "vmail.tpl.php");
				}
			}
		}
	 ...
	 */
	function get_vulnerability_report($vuln_array, $id_user, $use_filter=true, &$vulns_in_report=0) {
		global $global_db;
		global $MESSAGES;
